...
class OrdersRepoImpl implements OrderRepo {
  ...
  Stream<Either<Failures, List<OrderEntity>>> fetchOrders() async* {
    try {
      await for (var (data as List<Map<String, dynamic>>)
          in dataBaseServices.streamData(path: BackEndEndPoint.kGetOrders)) {
  ...
  @override
  Future<Either<Failures, List<OrderEntity>>> updateOrder({
    required OrderStatus status,
    required String orderId,
  }) async {
    try {
      await dataBaseServices.updateData(
        path: BackEndEndPoint.kUpdateOrder,
        data: {'status': status.name},
        documentId: orderId,
      );
      return Right([]);
    } catch (e) {
      return Left(Failures(errorMessage: 'failed to update data'));
    }
  }
}
